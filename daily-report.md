...
TAG: [BUG]
TITLE: Fix S3 attachment downloads opening in-browser instead of downloading
DESC: User reported that after S3 was wired up, clicking "download" on a chat/task attachment opens the file (e.g. as a raw S3-signed URL / in a new tab) instead of triggering a file download. Also asked to verify/implement "delete own message" across the repo.
Investigated via an Explore subagent covering both backend-py and frontend. Message delete was already fully implemented end-to-end: DELETE /chat/messages/{message_id} (backend-py/app/api/v1/chat.py) -> chat_service.delete_message() with author-only ownership check (message.author_id != user_id -> 403) plus workspace/channel/DM membership guard, hard delete (no soft-delete column on ChatMessage, unlike Workspace which does soft-delete), Socket.IO broadcast "chat:message:delete" so other connected clients remove it in real time, and frontend UI (delete menu item + confirm dialog + optimistic removal with rollback) in both ChatMessageRow/ConversationView and the ThreadPanel/ThreadMessageRow. No changes made here — nothing to build.
Root cause of the download bug: backend-py/app/services/s3_service.py presign_get() hardcoded `ResponseContentDisposition: 'inline; filename="..."'` on every presigned GET URL. Per HTTP semantics, an explicit server-sent Content-Disposition: inline header overrides the frontend's `<a href={downloadUrl} download>` attribute for cross-origin URLs (which a presigned S3 URL always is), so the browser opens/previews the file instead of downloading it — regardless of what the frontend did. presign_get is the single shared helper used by both attachment_service.py (chat attachments) and task_attachment_service.py (task attachments), so the bug affected every download button in the app from one place.
Fix: changed `inline` -> `attachment` in that one Content-Disposition value (s3_service.py). Verified this can't break the existing inline image-preview UX: every place that previews an image (MessageAttachmentList.tsx, AttachmentPreviewRow.tsx, ChannelFilesDialog.tsx, CommentAttachmentCard.tsx, TaskDrawer.tsx, InlineMessageEdit.tsx) renders it via `<img src={downloadUrl}>`, and `<img>` tags ignore the Content-Disposition header entirely (it only affects top-level navigation / explicit anchor downloads) — so previews keep working unchanged while the actual download links/buttons now force a save-as. Also checked for header-injection risk from interpolating the user-supplied file name into the header value: both attachment_service.py and task_attachment_service.py already run file names through a `_sanitize_filename` helper (regex-strips to `[a-zA-Z0-9._-]`, trims leading/trailing `._`, caps at 180 chars) before storing them, and that sanitized value is what reaches presign_get, so no injection path exists.
One-line change, no schema change, no new endpoint (per project convention of reusing existing code) — reviewed and approved by user, then committed and pushed to main.
...
TAG: [BUG]
TITLE: Fix chat composer paste formatting bloat and silent Chrome desktop notifications
DESC: User reported two separate issues: (1) pasting long text into the chat composer visually 2-3x's the message with extra blank lines/gaps, making it unreadable; (2) the notification sound plays for new messages but no Chrome/OS desktop popup ever appears.
Verified both via an Explore subagent pass before touching code, since the daily-report/CLAUDE.md convention is investigate-then-implement, not assume-then-fix.
Bug 1 root cause: frontend/src/components/chat/composer/RichComposerField.tsx handlePaste() reads clipboard text/plain and inserts it verbatim via document.execCommand("insertText", ...). Copying from Word/Google Docs/Notion routinely emits a blank line after every paragraph in their text/plain payload (this is inherent to those apps' plain-text serialization, not something the composer's own HTML-stripping needed to fix, since HTML clipboard data was never being read to begin with). Those blank lines were inserted untouched, and the send-time normalizer (normalizeComposerHtml -> extractPlainTextWithLineBreaks in frontend/src/lib/chat/rich-text/sanitize.ts) only trimmed trailing newlines, never collapsed interior runs of them.
Bug 2 root cause: frontend/src/stores/settings-store.ts has desktopNotifications defaulting to false while soundEnabled defaults to true — two fully independent flags. ChatSocketProvider.tsx gates the notification sound only on soundEnabled but gates the actual `new Notification(...)` popup (via showDesktopNotification in frontend/src/lib/notifications/desktop.ts) on desktopNotifications, which only ever flips to true through an explicit Settings toggle that itself calls Notification.requestPermission(). Nothing proactively asks a new user for permission, so out of the box every user hears the sound but never sees the OS popup. The underlying Notification API plumbing, permission handling, and click behavior were all already correct once enabled — this was a defaults/discoverability gap, not a broken pipeline.
Fixes implemented (approved by user, "production grade... best and smooth UX"):
(1) Added collapseBlankLines()/normalizePastedText() in sanitize.ts (collapses 3+ newlines down to one blank line, trims trailing whitespace per line) applied at paste time in both frontend/src/components/chat/composer/RichComposerField.tsx and the plain-textarea frontend/src/components/chat/mentions/MentionComposerField.tsx (used for task comments etc.), plus hardened extractPlainTextWithLineBreaks() with the same collapse as defense-in-depth for any other path that reaches it.
(2) Added frontend/src/components/providers/DesktopNotificationPrompt.tsx: a one-time, dismissible sonner toast ("Turn on desktop notifications?") shown 15s after login if the user hasn't already enabled or dismissed it and permission isn't already denied/unsupported, following the exact toast-with-action pattern already used by AutoUpdateProvider.tsx. Requesting permission still only happens on an explicit user click (Chrome will not honor an auto-triggered request), so this nudges rather than force-prompts. Added desktopNotificationPromptDismissed to settings-store.ts (zustand-persisted) so the nudge never nags twice. Wired into AppProviders.tsx next to ChatSocketProvider.
Test status: tsc --noEmit clean; vitest 42/45 passing (3 pre-existing unrelated sidebar-display-unread.test.ts failures, confirmed not in this changeset via git status).

TAG: [TASK]
PARENT: Fix chat composer paste formatting bloat and silent Chrome desktop notifications
TITLE: Add Markdown-to-rich-text conversion for pasted text
DESC: Follow-up user request: also let pasting Markdown (e.g. copied from a README or another chat) auto-convert into the composer's actual rich formatting instead of showing literal ** and # characters. Asked user two clarifying questions via AskUserQuestion first (both answered with the recommended option): convert into real rich formatting (bold/italic/headings/lists/etc.) rather than just stripping the syntax to clean plain text, and only convert when the pasted text actually looks like Markdown (auto-detected) rather than always running the converter — so plain prose like "5 - 3 = 2" is never misinterpreted as a list.
No Markdown parser existed in the repo (checked package.json and node_modules, confirmed none present, including transitively). Rather than adding an external dependency (marked/markdown-it), wrote a small scoped converter since the composer only supports a narrow, fixed tag set anyway (see ALLOWED_TAGS in sanitize.ts: bold/italic/strike/code/pre/h1-4/blockquote/ul/ol/li/a) — a full CommonMark implementation (tables, images, footnotes) would be unused surface area and unnecessary bundle weight.
Added frontend/src/lib/chat/rich-text/markdown-paste.ts: looksLikeMarkdown() regex-detects real Markdown syntax (bold/italic/strikethrough/inline code/headings/quotes/ordered+unordered lists/links) while avoiding false positives on ordinary punctuation. buildMarkdownHtml() parses into block types (heading/blockquote/fenced-code/list/paragraph) and renders inline formatting (bold/italic/strikethrough/inline-code/links), escaping all user text and restricting link hrefs to https/mailto at match time so raw HTML or javascript: URLs can never execute (verified with dedicated XSS-safety tests, not just informally). markdownToComposerHtml() wraps that with sanitizeMessageHtml (DOMPurify) as defense-in-depth for the real runtime path.
Added frontend/src/lib/chat/rich-text/dom.ts: insertHtmlAtCursor(), mirroring the existing insertTextAtCursor() but for inserting a pre-built HTML fragment at the caret.
Wired into RichComposerField.tsx's paste handler: if looksLikeMarkdown(text) is true, convert and insert as real formatting; otherwise falls through to the plain-text normalization path from the earlier fix. Not wired into MentionComposerField.tsx since that's a plain textarea with no rich-text rendering to convert into.
Testing note: the project's vitest.config.ts defaults to environment "node" (no DOM-dependent test in the repo previously, and jsdom isn't installed) — sanitizeMessageHtml/DOMPurify.sanitize requires a real DOM and can't run under plain Node. Rather than adding jsdom as a new test-only dependency, exported buildMarkdownHtml (the pure string-building logic, DOM-free) separately from markdownToComposerHtml (adds the DOMPurify pass, browser-only) so the parsing/escaping/link-safety logic is fully unit-testable without a DOM. Added markdown-paste.test.ts: 12 cases covering detection accuracy, every block/inline type, and the two XSS-safety guarantees. Two initial test assertions were wrong on first pass (checked for the substring "onerror"/"javascript:" appearing anywhere in output, but escaped-as-text is safe and expected) — corrected to assert the actually meaningful property (no live <img>/<a href="javascript:> tag survives), then all 12 passed.
Test status: tsc --noEmit clean; vitest src/lib/chat 54/57 passing (same 3 pre-existing unrelated sidebar-display-unread.test.ts failures as the parent bug fix, 12 new tests all passing).

TAG: [BUG]
TITLE: Fix production disk-full deploy failure (containerd snapshot dir ENOSPC) and its root cause
DESC: User reported a failed prod deploy: `web` container rollback loop with "failed to create prepare snapshot dir: ... no space left on device". Root VPS disk (/dev/root, 19G) was at 99% (18G used, 266M free).
Diagnosed via `docker system df -v`: build cache alone was 4.545GB, plus dozens of dangling `<none>` images going back 9+ days (many from superseded builds). Walked user through safe cleanup on the VPS directly (docker builder prune -af, docker image prune -af) — freed disk from 99% to 63% (18G -> 12G used, 6.9G free). Confirmed via `df -h /`.
Root cause of the accumulation, found by reading deploy/deploy.sh and deploy/deploy-admin.sh: both scripts' rollback safety net (`snapshot_image` tags the current image as `<name>-rollback` before every build) leaves the image it replaces behind as a dangling, untagged image every single deploy, and nothing was ever pruning them — so they piled up indefinitely (9+ days of history visible in `docker system df -v`) until the disk filled and took down a deploy that had nothing else wrong with it.
Fix: added an image/build-cache prune step to all three deploy scripts (deploy.sh, deploy-admin.sh, deploy-staging.sh — staging shares the same disk/Docker daemon as prod), placed right after `git reset --hard` and before any build. Uses `docker image prune -f` (no `-a`) which only removes genuinely dangling/unreferenced images — verified this cannot touch a running container's image or the just-tagged `-rollback` image, so the existing rollback safety net in all three scripts is unaffected — plus `docker builder prune -f --filter "until=72h"` for the build-cache half of the problem. Syntax-checked all three scripts with `bash -n` after editing.

TAG: [SUBTASK]
PARENT: Fix production disk-full deploy failure (containerd snapshot dir ENOSPC) and its root cause
TITLE: Fix Tauri Android build compile errors and stop Android build cache from being tracked in git
DESC: While investigating the deploy, noticed the latest commit on main (3bdb8be, "feat(android): initialize tauri v2 android target & build APK", not made by me — pre-existing user work) had a real Rust compile bug in frontend/src-tauri/src/lib.rs: `let builder = tauri::Builder::default()...` was declared immutable but then reassigned inside a `#[cfg(desktop)]` block (`builder = builder.plugin(...)`), which fails to compile with "cannot assign twice to immutable variable" on any desktop target. Also found via `cargo check` that the same commit had dropped the `use tauri::Manager;` import while keeping `app.get_webview_window(...)` (a Manager trait method) inside that same desktop-only block.
Fixed both: `let mut builder` with `#[allow(unused_mut)]` (mobile builds compile out the `#[cfg(desktop)]` block entirely, so `builder` is never mutated there — the allow avoids a warning on that target), and re-added `use tauri::Manager;` gated behind `#[cfg(desktop)]` (unconditional would warn as unused on mobile, since Manager is only used inside the desktop-only block). Verified with `cargo check` — clean compile, zero warnings, on the default (desktop) target. Attempted an Android cross-compile check too (`cargo check --target aarch64-linux-android`) but it failed on a missing NDK/clang toolchain on this Windows dev machine, unrelated to the code — the `#[cfg(desktop)]`/`#[cfg(mobile)]` gating itself is correct regardless, since the desktop-only block (and its Manager import) simply never compiles on the mobile target.
User separately flagged (mid-session) that Android build cache/artifacts must never be committed. Investigated frontend/src-tauri/gen/android/ (untracked, 259MB total): confirmed `.gradle/` (6.0MB), `app/build/` (251MB — the bulk of it), `build/` (140K), `buildSrc/.gradle/` (84K), `buildSrc/build/` (791K) are all pure Gradle build outputs/caches that regenerate on every build. Tauri's own `tauri android init` had already generated a correct nested gen/android/.gitignore covering these (confirmed via `git add -n` dry-run: only 40 genuine files — Kotlin/Java source, AndroidManifest.xml, resources, Gradle wrapper jar/config, build.gradle.kts — would ever be staged, zero cache dirs). Added defense-in-depth rules to frontend/.gitignore anyway (src-tauri/gen/android/.gradle/, */build/, local.properties, *.iml, .idea/) in case that nested .gitignore is ever missing or regenerated differently before caches exist — re-ran the dry-run after adding them and got the identical 40-file result, confirming no real source was accidentally excluded.
Committed the genuine Android config/source that IS required for the build to work on a fresh checkout (user explicitly flagged this after I initially planned to leave it all uncommitted as "the user's own in-progress work" — correctly redirected to think about what the server actually needs): frontend/src-tauri/gen/android/ (the 40 real source/config files, cache excluded by gitignore), gen/schemas/android-schema.json and mobile-schema.json (new — Tauri's per-platform capability/permission schema, same pattern as the already-tracked desktop-schema.json/windows-schema.json), and gen/schemas/capabilities.json (added the `core:window:default` permission, required for the app to have window permissions on the new Android target). Verified Cargo.toml had no actual content diff (line-ending normalization only, git left it unstaged).

TAG: [FEATURE]
TITLE: Set up signed Android APK builds via CI and audit app-wide mobile responsiveness
DESC: User reported the existing kinetix.apk on their Desktop was "corrupted" when installing on a real device, and separately asked for a full mobile-responsiveness pass across the whole app (not just chat, which two recent commits — bbf3960, 3300485 — already claimed to make "100% mobile responsive").
Root-caused the "corrupted" APK: confirmed no signing keystore existed anywhere in the repo (frontend/src-tauri/gen/android/app/build.gradle.kts had no `signingConfigs` block at all), and the only APKs that had ever been built were literally named app-arm64-release-unsigned.apk / app-universal-release-unsigned.apk under gen/android/app/build/outputs/apk/. Android refuses to install an unsigned release APK (or shows it as corrupted/"app not installed"), which matches the report exactly.
Checked this machine for a local build path first: Android Studio is installed but its NDK/build-tools/platforms folders are empty placeholder directories (not actually installed), so a local `gradlew`/`cargo` Android build isn't possible here without the user installing several GB of SDK components. Asked user how to proceed (AskUserQuestion) — chose CI (GitHub Actions) over local setup, since CI runners already have a full, known-good SDK/NDK.
Generated a new Android release signing keystore (kinetix-release.jks, RSA 2048, PKCS12, alias "kinetix", valid 30 years to 2056-08-25) using keytool from Android Studio's bundled JBR (the only working Java on this machine). Sent the .jks file and a passwords.txt (with a CRITICAL warning that every future release must be signed with this same keystore or Android will refuse to install updates over the existing app) to the user via SendUserFile for durable storage, then deleted the intermediate base64 copy from the scratchpad. Confirmed with user before writing anything to GitHub, then set 4 repo secrets via `gh secret set` on coding1100/kinetix: ANDROID_KEYSTORE_BASE64, ANDROID_KEYSTORE_PASSWORD, ANDROID_KEY_ALIAS, ANDROID_KEY_PASSWORD.
Added the missing signingConfigs.release block to gen/android/app/build.gradle.kts, reading from a key.properties file (gitignored, matching the pattern gen/android/.gitignore already anticipated but that was never actually wired to anything) — falls back to no signing config (producing an unsigned APK, same as before) if key.properties is absent, so local dev builds without the keystore still work as before, just unsigned.
Found and fixed a second real bug while wiring this up: Cargo.toml had `tauri-plugin-single-instance = "2"` as an unconditional dependency, but that crate does not support mobile targets at all (desktop-only, matching the #[cfg(desktop)] gating already applied to its usage in lib.rs during the prior session's compile fix) — moved it into a `[target.'cfg(not(any(target_os = "android", target_os = "ios")))'.dependencies]` block so `cargo build` can actually resolve for an Android target. Re-verified desktop `cargo check` stays clean after the change.
Added .github/workflows/build-android.yml: Ubuntu runner, Node 22 + Java 17 (Temurin) + Android SDK (android-actions/setup-android) + NDK 27.0.12077973 + Rust with all 4 Android targets, writes the keystore/key.properties from secrets, runs `npx tauri android build --apk` (Tauri's own Android build wrapper, which already existed as a devDependency — no new CLI install needed), verifies the located APK isn't the *-unsigned.apk variant before uploading, uploads it as a build artifact (90-day retention), and deletes the keystore/key.properties in an `if: always()` cleanup step regardless of build outcome. Triggers on push to main touching frontend/src-tauri/** or the workflow file itself, plus workflow_dispatch for on-demand builds. Followed the existing release-desktop.yml workflow's conventions (secrets naming pattern, Node/Rust setup steps).
Added explicit .gitignore rules (frontend/.gitignore) for *.jks/*.keystore/key.properties/keystore.properties under gen/android/app/ specifically, as defense-in-depth beyond the nested gen/android/.gitignore, since this is exactly where CI writes the real private signing key during a build.
Launched a background Explore subagent to audit mobile responsiveness across every app area (chat, home, spaces/TaskDrawer, teams, settings, shell/nav, auth, modals) — specifically checking whether the "100% mobile responsive" claim from the earlier commits holds up, or if it only covered chat.

TAG: [TASK]
PARENT: Set up signed Android APK builds via CI and audit app-wide mobile responsiveness
TITLE: Fix real mobile-responsiveness bugs found by the audit
DESC: Audit came back: the "100% mobile responsive" claim (bbf3960, 3300485) was overstated. It correctly fixed the global shell (TopBar/GlobalNav/MobileBottomNav), Chat's conversation header/composer, Chat/Home sidebars on their root routes, BoardView columns, and Spaces toolbar tab scrolling — but multiple major areas were untouched, and some of the touched infrastructure was silently bypassed by its own consumers. Fixed the high/very-high-confidence findings:
1. Chat side panels (ThreadPanel, PersonProfilePanel, ChannelDetailsPanel, DmGroupMembersPanel, DmPersonSettingsPanel, DmRepliesPanel, DmSearchPanel) all passed fixed-px `widthClass` (e.g. w-[380px]/w-[400px]) and old-style `marginClassName` overrides to the shared PanelCardShell component, completely defeating the mobile full-screen-sheet default it was given in bbf3960 — every "detail rail" in chat was unusable on mobile despite the shell itself being correct. Root-caused and fixed at the shell level: renamed the prop to `marginSpacingClassName` (a breaking rename, forcing every call site to be touched and verified, not just the obvious ones) and split it into an always-applied mobile-safe overlay wrapper vs. caller-customizable desktop-only spacing: a caller can no longer opt out of the mobile behavior by accident. Updated widthClass everywhere to be `md:`-prefixed only. Verified via grep that no consumer still references the old prop name.
2. SpacesSidebar (frontend/src/components/spaces/SpacesSidebar.tsx) had no mobile gating at all — fixed 260px width, always shown, unlike ChatSidebar/HomeSidebar which got isChatRoot/isHomeRoot gating in the prior commit. Asked user (AskUserQuestion) whether to also add a Spaces bottom-nav tab or just fix the sidebar's own behavior — user chose sidebar-only, no nav changes. Since /spaces itself immediately redirects to the first list (no stable "browse" landing state, unlike /home or /chat root), gated on `!pathname.startsWith("/spaces/l/")` instead of an exact-route match: full-width on /spaces (briefly, during its own redirect, and for the empty-state case), hidden below md once inside a list.
3. Added a mobile-only back button to SpacesListToolbar.tsx (shared by both /spaces/l/[listId] and /home/l/[listId] via ListWorkspace.tsx's existing basePath prop pattern) so users aren't stranded once the sidebar hides — threaded a new `backHref` prop through ListWorkspace so it correctly returns to /spaces or /home depending on which route mounted it.
4. Fixed the same toolbar's tab-bar-vs-action-buttons overflow bug (SpacesListToolbar.tsx) — the 8 view-mode tabs and the Add task/Share buttons were both inside one `overflow-x-auto` row, so `justify-between` had no effect once it overflowed and the action buttons scrolled out of reach. Split into two flex children: only the tabs scroll (own `min-w-0 overflow-x-auto` wrapper), the action buttons stay in a `shrink-0` sibling, always visible.
5. Fixed Home's dead-end navigation: HomeSidebar hides itself below md on every /home/* route except /home and /home/inbox, but 6 sub-routes (my-tasks, spaces/[id], channels, favorites, l/[listId], all-tasks) had no back button at all, stranding mobile users. Added a new frontend/src/components/shell/HomeMobileBackBar.tsx, mounted once in home/layout.tsx (a server component, so this had to be a separate small client component reading usePathname rather than inline logic) — renders a single "Back to Home" bar for exactly the affected routes; /home/l/[listId] is excluded since it already gets its own contextual back button from fix #3 above (backHref="/home").
6. Chat message action toolbar (reply/react/more, in ChatMessageRow.tsx and ThreadMessageRow.tsx) was `group-hover`-only with zero touch fallback — undiscoverable on any touch device, and the primary way to reply/react/edit/delete/pin a message. Same bug, worse (no fallback at all, unlike chat's toolbar which at least shows once its own dropdown is open), in Home's InboxFeedRow.tsx Snooze/Clear buttons on the default landing page. Fixed both using Tailwind v4's built-in `pointer-coarse:` variant (confirmed tailwindcss ^4 is installed) alongside the existing `group-hover:` classes — shows the actions permanently on touch/coarse-pointer devices without affecting desktop mouse behavior, since pointer-coarse targets actual touch hardware rather than viewport width.
7. TaskDrawer.tsx had two real overflow bugs: the two-column body (task detail + Activity/comments rail) was a plain `flex` row with `min-w-[400px]` on the Activity column and no mobile fallback — exceeds any phone viewport outright. Changed the row to `flex-col md:flex-row` (stacks vertically on mobile, Activity panel gets `min-h-[50vh]` so it's not squished, full side-by-side layout restored at md) and moved the width/min-width constraint to `md:` only. Also fixed the Share-task dialog's unconditional `min-w-[600px]` (TaskDrawer.tsx:2871) the same way — gated to `md:`.
8. TeamsSidebar.tsx had the identical fixed-260px-no-mobile-gating bug as SpacesSidebar (currently unreachable in production since FEATURE_FLAGS.teams is false, but fixed anyway since it's real code that will ship). Mirrored the isTeamsRoot pattern at the `lg` breakpoint (matching its existing `!secondaryPanelOpen && "hidden lg:flex"` collapse behavior, not `md` like the other sidebars).
9. TopBar.tsx had no safe-area padding at all despite `pt-safe`/`pb-safe`/`pl-safe`/`pr-safe` utilities existing in globals.css — audit found only `pb-safe` was ever actually used anywhere (MobileBottomNav, MessageComposer), the rest were dead code. Fixed the concrete gap: added `pt-safe` to TopBar and changed its fixed `h-11` to `min-h-11` (required — padding alone would add space but a fixed height would then clip content under a notch instead of growing to accommodate it). Left pl-safe/pr-safe/mb-safe unused — no concrete landscape-notch symptom found to justify applying them speculatively.
Deliberately NOT fixed in this pass (lower confidence/severity or out of scope per user's answer): CalendarView's grid-cols-7 (cosmetically cramped but doesn't overflow, would need a real mobile agenda-view redesign, not a quick class fix), GanttView's wide scrollable table (flagged by the audit itself as likely intentional), ComposerMentionHoverPeek and InlineMessageEdit's hover-only remove-attachment button (low severity, non-blocking affordances).
Verified: `tsc --noEmit` clean across all 19 changed files. Full vitest suite: 87/90 passing (same 3 pre-existing unrelated sidebar-display-unread.test.ts failures noted throughout this session, confirmed not touched by this changeset).
...
TAG: [FEATURE]
TITLE: Add real login/notification sound files, fix loudness, and fix Windows taskbar badge
DESC: User provided two WAV files (login-sound.wav, notification-sound.wav) at the repo root and asked to: (1) play login-sound.wav on every successful login, web and desktop; (2) add notification-sound.wav as a new sound preset and make it the default for every user (existing and new); (3) make sure sound is as loud as possible; (4) fix the Windows taskbar notification-count badge, which the user reported not showing up at all in the desktop app.
Root cause of "sound is not persistent" (raised in the request framing): the entire existing notification sound system (frontend/src/lib/notifications/sound.ts) synthesizes tones via raw Web Audio oscillators (no actual audio files at all) - every preset is 0.07-0.42 seconds long, which is why it read as cutting off immediately rather than being a real, sustained notification sound.
Moved both files into frontend/public/sounds/ (already-established convention - found and removed a stale, already-unused notification.wav leftover from an earlier attempt at this same idea). Files must live under public/ and be served from the app's own origin because the desktop build's CSP (media-src 'self' ...) and frontendDist point at the live production site (https://kinetix.mindrind.com) - the Tauri app IS that site in a webview, so this works identically for web and desktop with no separate bundling needed.
Measured actual peak loudness of both files with a small one-off Node script reading each WAV's data chunk directly (no ffmpeg/sox available) rather than guessing: login-sound.wav already peaks at 0dBFS (zero headroom - any boost would clip), notification-sound.wav peaks at -1dBFS (89.1% of max, ~12% headroom). Implemented file playback in sound.ts by routing through the SAME AudioContext already used for the synthesized presets (via a GainNode) rather than a plain <audio> element, specifically so the GAIN could be pushed above the plain element's hard 1.0 volume cap where the source has headroom - notification-sound.wav gets a measured 1.12x gain (fills the headroom without clipping), login-sound.wav gets 1.0 (already maxed, boosting would only distort). Added the new preset as "loud-alert" (label "Loud Alert (default)") in SOUND_PRESETS/SoundPreset, and a separate playLoginSound() export (not tied to the notification sound toggle/preset at all - login is a distinct always-on cue, not a "new message" event) wired into both frontend/src/app/auth/login/page.tsx and .../auth/oauth/callback/page.tsx right after their existing toast.success(...) login-confirmed points.
"Default for every user" required more than just changing the store's initial value, since soundPreset lives in a zustand `persist` store (localStorage) - existing users already have "chime" persisted and would never see a changed initial value. Found the exact versioned-migration pattern already used elsewhere in the codebase (chat-store.ts, home-sidebar-store.ts) and applied the same approach: bumped settings-store's persist version to 2 with a migrate() that flips soundPreset from "chime" -> "loud-alert" ONLY if it's still exactly the old default value, preserving any user's deliberate choice of pop/ping/soft/bell/breeze untouched.
Root-caused the Windows taskbar badge separately (not something either of us had touched before): frontend/src/lib/notifications/badge.ts only ever called Tauri's `setBadgeCount`, whose own official docs literally state "Windows: Unsupported. Use setOverlayIcon instead" - this API is macOS-dock/Linux-only and was always silently no-op-ing on Windows. Also found and fixed a SECOND independent reason it could never have worked on any platform: Tauri v2's permission system never actually granted either `set_badge_count` or `set_overlay_icon` - neither is part of `core:window:default` (verified directly against gen/schemas/acl-manifests.json's default_permission list for core:window), so even a correct API call would have been silently blocked by the capability system. Added `core:window:allow-set-badge-count` and `core:window:allow-set-overlay-icon` to the real source-of-truth capabilities/default.json (NOT the generated gen/schemas/capabilities.json, which is derived output - regenerated it via `cargo check` to confirm the change flows through correctly).
Implemented the actual Windows fix: added `setOverlayIcon` alongside the existing `setBadgeCount` call in badge.ts (kept both - setBadgeCount is still correct for macOS/Linux per the same docs, and is harmless to leave calling on Windows since it just fails silently there). Refactored the existing favicon canvas-badge-drawing code (already proven, already drawing a red circle + count) into a shared drawBadgeCircle() helper instead of writing new drawing logic, and added buildOverlayIconBytes() which draws the same badge onto a small transparent 64x64 canvas and exports it as PNG bytes via canvas.toBlob(). Required enabling the `image-png` Cargo feature on the `tauri` dependency (was `features: []`) so `Image.fromBytes()` can actually decode the PNG bytes into the RGBA format setOverlayIcon needs - verified with `cargo check` before and after, clean both times; Cargo.lock picked up ~36 lines of new PNG-decoding transitive deps as expected.
Verified: `tsc --noEmit` clean, `cargo check` clean (both before adding image-png and after), full vitest suite 87/90 passing (same 3 pre-existing unrelated sidebar-display-unread.test.ts failures from earlier in this repo's history, confirmed unrelated). Pushed to main (45bdefe..39c1a55) after user asked to push.

TAG: [TASK]
PARENT: Add real login/notification sound files, fix loudness, and fix Windows taskbar badge
TITLE: Add a minimal send-message confirmation sound
DESC: User asked for a subtle sound on message send ("gives a premium feel") - same UX pattern as Slack/iMessage/Telegram's send tick.
Found the two places a chat message actually gets sent client-side: ConversationView.tsx's handleSend (main channel/DM messages) and ThreadPanel.tsx's handleReply (thread replies) - both call the shared createOptimisticMessage() from lib/chat/messages.ts at the exact moment of send, before the network request resolves. Deliberately did NOT put the sound call inside createOptimisticMessage() itself despite it being the single common point both routes share: it's a pure data-construction function with existing unit test coverage (messages.test.ts) and no other side effects, and reaching into the audio/browser layer from a lib module would be a layering violation plus break its testability outside a DOM/AudioContext. Instead added playSendSound() at the two actual call sites, right where the optimistic message is added to local state (immediate feedback, not delayed by the round-trip).
Added playSendSound() to notifications/sound.ts as a new synthesized tone, NOT reusing the loud-alert file or any existing notification preset - sending your own message should never compete in volume with the loud incoming-message alert, so this is a deliberately tiny, quick (70ms) high-pitched blip at fixed low gain (0.09), independent of the user's chosen notification sound/volume preset. Still respects the soundEnabled toggle since it's a routine action sound, not a critical alert.
Verified: `tsc --noEmit` clean.

TAG: [TASK]
PARENT: Add real login/notification sound files, fix loudness, and fix Windows taskbar badge
TITLE: Build and ship a signed desktop release to verify the Windows taskbar badge fix
DESC: User reported the taskbar badge still wasn't showing after the earlier badge.ts fix was merged. Explained why: that fix is native Rust/Tauri code, and the desktop app is a compiled binary the user's already-installed app predates - unlike the web/JS changes (which reach every user automatically since the desktop app loads the live production site), this specific fix requires a new installer.
Triggered .github/workflows/release-desktop.yml via `gh workflow run` (workflow_dispatch) rather than waiting for a version-tag push. Monitored the run (34601045322) in the background; all 3 platforms (Windows, macOS, Ubuntu) succeeded in ~13 minutes. Retrieved the draft release's Windows installer asset URL directly via `gh release view --json assets` and gave the user a direct download link, since the release itself is a draft (not publicly listed, but downloadable by repo collaborators).
User asked whether every user needs to reinstall the .exe for future updates too - explained the distinction clearly: pure web/JS changes (sounds, mobile responsiveness, sidebar features) reach everyone automatically via the live site the desktop app loads; only native-layer changes (new Cargo features, capability/permission grants, Rust code) require a new installer, same as the Android APK situation from earlier in the week.

TAG: [FEATURE]
TITLE: Add "jump to unread" pill to Chat and Home sidebars
DESC: User asked for a ClickUp-style floating pill that appears when an unread conversation/item is scrolled out of view in the sidebar, letting the user click it to jump straight to that item - shared a screenshot of ClickUp's own "↓ Unread messages" pill for reference. Confirmed scope via AskUserQuestion: both Chat's channel/DM sidebar AND Home's sidebar (which has its own separate unread badges on the Inbox nav item, channels, and DMs) - not just the task-count numbers shown in the screenshot, since those are a different, unrelated data source (open task count per list, not unread activity).
Investigated the existing sidebar structure first rather than assuming: ChatSidebar.tsx and HomeSidebar.tsx each have their own ChannelRow/DmRow leaf components (4 total across both files, plus HomeSidebar's top-level HomeNavItem for the Inbox badge) rendered from several different list-mapping call sites (favorites/channels/DMs, recents view, organized view, main nav items, Spaces "shared with me" entries) - too many call sites to thread refs through cleanly. Instead tagged only the actual leaf row components (the ones that render the unread badge) with `id` + `data-unread-anchor="true"` when unread, and built a query-selector-based hook rather than a ref-registration pattern, so adding this to any future new row type is a one-line change instead of new plumbing.
Added frontend/src/hooks/use-off-screen-unread.ts: takes a scroll container ref, and via a combination of `getBoundingClientRect()` comparisons (against the container's own rect) plus a MutationObserver (rows appear/disappear/reorder without necessarily firing a scroll event - new message arrives, filter changes, list re-sorts) and scroll/resize listeners, computes hasUnreadAbove/hasUnreadBelow. Exposes scrollToNearestAbove/Below which finds the CLOSEST off-screen unread anchor in that direction (not just the first/last one in DOM order) and calls scrollIntoView with a brief CSS highlight flash (added a shared @keyframes sidebar-unread-jump-flash to globals.css, using existing --sidebar-accent/--radius-md tokens for both themes).
Added frontend/src/components/shared/UnreadJumpPill.tsx: renders up to two floating pills (top "↑ Unread messages" / bottom "↓ Unread messages") absolutely positioned over the scroll container's edges, only when that direction's boolean is true.
The existing ScrollArea wrapper (components/ui/scroll-area.tsx, built on @base-ui/react's scroll-area primitive) didn't forward a ref to the actual scrollable viewport element - added an optional `viewportRef` prop (backward compatible, every existing caller unaffected) since @base-ui's own ScrollAreaViewport already forwards a ref internally, confirmed via its .d.ts.
Wired into both sidebars: ChatSidebar.tsx's ChannelRow/DmRow (keyed by channelId/dmId) and HomeSidebar.tsx's ChannelRow/DmRow (keyed by `id`) plus HomeNavItem (keyed by item.id, for the Inbox unread badge specifically - the only nav item with a real "unread" concept, matching the existing `item.id === "inbox"` badge-gating already in that component). Verified `id`/`data-*` actually reach the real DOM node: ContextMenuTrigger (used by all 4 ChannelRow/DmRow variants) is base-ui's own primitive typed as `BaseUIComponentProps<'div', ...>`, which extends standard div props and renders a real `<div>` - confirmed via its own .d.ts rather than assuming.
Verified: `tsc --noEmit` clean, full vitest suite 87/90 passing (same 3 pre-existing unrelated sidebar-display-unread.test.ts failures as every other check this session).

TAG: [FEATURE]
TITLE: Fix the desktop auto-update pipeline (was completely non-functional, discovered while discussing the taskbar-badge reinstall problem)
DESC: User asked, in the context of "do I need every user to reinstall the .exe again": whether Kinetix could silently auto-update itself in the background so future fixes never require manually redistributing an installer. Investigated rather than assumed - found the UX/frontend half was ALREADY built (frontend/src/components/providers/AutoUpdateProvider.tsx: checks for a native update once per session on launch via @tauri-apps/plugin-updater's check(), downloads+installs it silently in the background with no user action needed, then shows a "Restart Now" toast only for the unavoidable restart-to-apply step) - so the passive/automatic UX pattern the user asked for already existed in code and needed no new UI work.
Root-caused why it had never actually worked end-to-end: pulled the build logs from the desktop release run and found `Signature not found for the updater JSON. Skipping upload...` on all 3 platforms. TAURI_SIGNING_PRIVATE_KEY secret was set, but minisign private keys are virtually always password-protected, and there was no TAURI_SIGNING_PRIVATE_KEY_PASSWORD secret at all - tauri-action correctly refused to publish an unsigned/unverifiable update manifest rather than silently producing one. Asked user if the original password was known (AskUserQuestion) - it wasn't (likely lost from whenever the key was first generated, before this session's history) - so generated a brand new minisign keypair via `npx tauri signer generate` (the officially correct tool, not raw minisign) with a fresh random password, updated tauri.conf.json's updater pubkey to match, set both TAURI_SIGNING_PRIVATE_KEY (overwriting the old broken one) and TAURI_SIGNING_PRIVATE_KEY_PASSWORD as repo secrets, and added the missing `TAURI_SIGNING_PRIVATE_KEY_PASSWORD: ${{ secrets.TAURI_SIGNING_PRIVATE_KEY_PASSWORD }}` line to release-desktop.yml's env block (was never there - the literal missing piece). Flagged the unavoidable consequence: any already-installed desktop app has the OLD pubkey compiled in and can never verify updates signed with the new key, so those users need one more manual reinstall (the Windows build already provided earlier this session) - after that, auto-update works going forward under the new key.
Found and fixed a second, independent problem while investigating: backend-py/app/api/v1/desktop.py (the actual endpoint tauri.conf.json's updater.endpoints points at - a custom backend proxy, not GitHub's raw latest.json) was fully hardcoded (LATEST_NATIVE_VERSION = "0.1.4", a hardcoded URL, and LATEST_RELEASE_SIGNATURE = "" - an empty string, which would have made signature verification fail even if everything else worked). Asked user how they wanted this fixed (AskUserQuestion) - chose keeping the custom backend endpoint (rather than pointing Tauri directly at GitHub) specifically so publishing/un-drafting a GitHub release remains the deliberate rollout gate, rather than every CI build going live to all users instantly.
Rewrote desktop.py to dynamically fetch the most recently PUBLISHED (non-draft, non-prerelease) GitHub release via the GitHub API, find its `latest.json` asset (auto-generated and uploaded by tauri-action once signing works - confirmed via tauri-action's own README that `uploadUpdaterJson`/`uploadUpdaterSignatures` both default to true and that its documented matrix-build pattern already merges entries from multiple platform jobs into one manifest, so no manual JSON-merging logic was needed on the Python side), and re-serves the platform-matching entry from it - draft releases are never surfaced, so un-drafting a release on GitHub IS the rollout gate, unchanged from the user's existing review workflow. Added a `_TARGET_ALIASES` map (windows-x86_64, darwin-x86_64/aarch64 -> darwin-universal fallback, linux-x86_64) since the matrix build's universal macOS target may write a single `darwin-universal` key rather than per-arch keys (confirmed via the build log's `updaterJsonKeepUniversal: false` setting) and Tauri's own {{target}} URL placeholder needs to resolve to whichever key is actually present. Added a simple in-process TTL cache (5 min default, app/config.py's new `desktop_update_cache_seconds`) that also serves the last-known-good manifest on a GitHub API error rather than breaking the update check entirely during a transient outage. Added `github_repo`/`github_token` to config.py and docker-compose.env.example (token optional - repo is public, but raises the GitHub API rate limit from 60/hr to 5000/hr and would allow reading drafts if ever needed for internal testing).
Added backend-py/tests/test_desktop_update.py (7 cases, all passing): version parsing/comparison, 204 on already-up-to-date and on no-manifest-available, correct manifest returned for a matching target, darwin-universal alias resolution, 204 when the target isn't in the manifest at all, and that draft/prerelease releases are correctly skipped when picking "latest" (mocks httpx.AsyncClient directly rather than needing the full DB-backed test fixtures, since this endpoint has no DB dependency).
Verified: backend endpoint imports cleanly, all 7 new tests + 2 existing auth tests pass. Not yet triggered a new desktop release with the fixed signing - that's the natural next step to confirm the full pipeline (sign -> upload latest.json -> backend proxies it -> AutoUpdateProvider picks it up) actually works end to end.

TAG: [BUG]
TITLE: Investigate and resolve a live-production mobile-layout screenshot (full desktop sidebar rendering on phone)
DESC: User shared a screenshot of kinetix.mindrind.com open in what appeared to be Safari on an iPhone, showing the full desktop nav rail + channel sidebar rendered at full width on a chat page, squeezing the actual conversation into a narrow sliver - despite an earlier session's mobile-responsive pass (and its own follow-up bug-fix pass) having specifically fixed this exact ChatSidebar/GlobalNav hide-below-md behavior over a week earlier. User then said "make sure the whole platform, each and every page is mobile responsive - every single page" (not a sampled audit).
Launched a background Explore agent to do two things: (1) definitively root-cause the screenshot given the code, git history, and CI deploy logs, and (2) exhaustively audit all ~60 page routes in frontend/src/app. On (1), the agent found the fix genuinely live and correct in code (ChatSidebar.tsx isChatRoot gating, matching GlobalNav's hidden md:flex and MobileBottomNav's md:hidden), confirmed via `gh run list` that every deploy since the fix (including bbf3960 itself) succeeded, and confirmed no hydration mismatch, no duplicate/legacy route, and correct viewport meta - concluding the bug wasn't reproducible from the code and asking the user to verify their Safari tab wasn t stale/cached. Rather than accept that as the final word, asked the user directly whether iOS Safari's per-site "Request Desktop Website" toggle was active for the domain (that setting makes Safari report a fake ~980px viewport regardless of actual screen size, which would produce exactly the screenshot's symptom while still showing a real phone status bar) - user confirmed it was ON. Root cause fully resolved: not a code bug at all, a per-device Safari setting. No code change needed for this specific finding, but it's exactly the kind of thing worth having ruled out definitively rather than chasing a phantom bug.

TAG: [TASK]
PARENT: Investigate and resolve a live-production mobile-layout screenshot (full desktop sidebar rendering on phone)
TITLE: Fix the real mobile-responsiveness issues the full-platform audit found
DESC: Independent of the screenshot's real cause, the same background audit (and my own follow-up verification of every item it flagged, rather than trusting its conclusions blindly) found several genuine, unrelated mobile bugs across the app - fixed all of them:
1. frontend/src/components/home/InboxFeedRow.tsx - the Inbox's row layout was an unconditional 3-column grid (`grid-cols-[minmax(0,1.1fr)_minmax(0,1.6fr)_150px]`) with no mobile fallback, guaranteed to clip on any phone width. Changed to grid-cols-1 (stacked: title row, then preview+time row) below md, restoring the original 3-column grid at md: and up. Also removed the now-redundant pointer-coarse touch-visibility hack on the Snooze/Clear buttons since they're simply always in their own visible row on mobile now, simpler and more correct than layering touch-detection on top of a broken grid.
2. frontend/src/components/home/MyTasksTaskRow.tsx (used by all 7 My Tasks sub-routes) and frontend/src/components/spaces/ListTaskRow.tsx (used by every List-view task list in Home and Spaces) had the identical bug pattern - fixed-px multi-column grids (100px/110px/72-88px columns) with a paired column-header component that would look broken alone once rows stack. Fixed both the same way: grid-cols-1 stacked card layout below md (name+status on top, then a flex-wrap row of priority/due-date/assignee/comment-count meta below it, using `md:contents` on the wrapper so those same elements become real grid cells again at md: and up - avoids maintaining two separate markup trees for the two layouts), and hid both column-header components below md (a header row makes no sense once content stacks into cards).
3. Also fixed a hover-only touch-discoverability bug spotted in ListTaskRow.tsx while already in the file: the row's ⋮ actions menu was opacity-0 group-hover-only, unreachable on touch - same pattern as several already-known-and-fixed bugs from an earlier session. Rather than fix it in isolation, grepped the ENTIRE frontend for every remaining `group-hover:opacity-100` (and named-group variants like group-hover/list, group-hover/space, group-hover/folder, group-hover/comment, group-hover/follower, group-hover/image) to find every other instance of this same bug class in one pass instead of finding them one at a time across future sessions. Found and fixed 13 more real instances across frontend/src/app/(app)/home/assigned-comments/page.tsx (Resolve button), frontend/src/components/shell/HomeSidebar.tsx (4 instances - space/folder/list action menus, "New list" button, and the space-row expand chevron hint), frontend/src/components/spaces/SpacesSidebar.tsx (list action menu), frontend/src/components/spaces/TaskDrawer.tsx (3 instances - checklist options menu, subtask remove-ish icon, follower list unfollow icon), frontend/src/components/spaces/CreateTaskDialog.tsx (2 instances - remove-subtask button, item-options menu), frontend/src/components/tasks/TaskActivityComment.tsx (comment actions menu), and frontend/src/components/tasks/CommentAttachmentCard.tsx (download button). Each got the same `pointer-coarse:opacity-100` fix pattern already established in an earlier session (Tailwind v4's built-in touch/coarse-pointer media variant) rather than a bespoke fix per file. Deliberately did NOT touch decorative-only hover effects with no functional gap (zoom-in preview icons on image thumbnails where the whole image is already clickable, a purely aria-hidden arrow icon on TeamCard) - only fixed cases where the hidden element was the ONLY way to trigger a real, distinct action.
4. frontend/src/components/teams/TeamsSidebar.tsx - fixed an inconsistent lg: (1024px) breakpoint left over from an earlier session's fix (every other primary sidebar uses md:/768px) to md: for consistency, since Teams will eventually ship (currently FEATURE_FLAGS.teams = false, so zero current user impact, but worth fixing before the flag flips rather than after).
5. Systematically verified every remaining item flagged as "not independently confirmed" by the audit (11 pages: assigned-comments, channels, chat-activity, drafts-sent, favorites, list settings, custom section, space detail, space settings, invite/accept, and the root redirect page) plus did fresh broad greps across the ENTIRE frontend for the same anti-pattern signatures (unconditional grid-cols-[...px...], unconditional min-w-[Npx], dialogs/sheets with unconditional min-width) to make sure nothing was missed beyond what the audit happened to open. Found no further real breaks - remaining fixed-width usages (small avatar/badge sizes, search-input min-widths, dropdown-menu min-widths, three data tables in AllChannelsView/PeopleView/TeamsInsightsView) are all either non-layout-risk sizes or already correctly wrapped in overflow-x-auto (functional, if not pixel-perfect touch UX - left as a lower-priority polish item, consistent with how the same tables were scoped in an earlier session).
Also cosmetically refined the sidebar "jump to unread" pill (frontend/src/components/shared/UnreadJumpPill.tsx, built earlier this session) after the user said it looked chunky compared to ClickUp's reference - reduced padding/font-size/icon-size, switched to backdrop-blur-sm + bg-primary/95 for a lighter/more translucent look matching the reference more closely, shortened the default label from "Unread messages" to "Unread".
Verified: `tsc --noEmit` clean, full vitest suite 87/90 passing (same 3 pre-existing unrelated sidebar-display-unread.test.ts failures noted throughout every check this session). All fixes kept LOCAL/uncommitted-to-push per user's explicit "don't push until I say" instruction repeated during this task, separate from the version-bump commit below which the user did approve pushing.

TAG: [TASK]
PARENT: Fix the desktop auto-update pipeline (was completely non-functional, discovered while discussing the taskbar-badge reinstall problem)
TITLE: Bump to v0.1.4 and trigger the first release build with the fixed signing pipeline
DESC: User asked why no "update available" toast had appeared after the signing/backend fixes from earlier in this session. Explained precisely why: the backend endpoint only ever surfaces a PUBLISHED (non-draft) release as an update source (the deliberate rollout gate built earlier), and zero releases were published yet (both v0.1.2 and v0.1.3 are still drafts) - and separately, even a draft release had no latest.json/signature on it yet since no build had run since the signing fix. Confirmed via `gh release view` that no latest.json exists on any release.
Bumped version to 0.1.4 in three places that all needed to agree (tauri.conf.json, Cargo.toml, package.json - confirmed via `cargo check` that Cargo.lock regenerated cleanly), since the updater only ever offers something strictly newer than the currently-running version. User approved pushing ONLY this version-bump commit (kept separate/isolated from the still-local mobile-responsiveness fixes) since GitHub Actions builds from the pushed commit, not local files. Pushed (617d9b3..4ac34a7), then triggered release-desktop.yml via `gh workflow run` (run 34604239872). Also cleaned up a stray .playwright-mcp/ directory that got created from an aborted browser-tool call earlier in the session (user asked not to use browser testing) before it could accidentally get committed.
Monitoring the build in the background - once it completes, the next step is confirming (via build logs) that "Signature not found" no longer appears and a real latest.json gets uploaded to the draft release, then the user can publish that release on GitHub to make v0.1.4 live to the updater for anyone still on the old (broken-badge) desktop build.

TAG: [BUG]
PARENT: Fix the desktop auto-update pipeline (was completely non-functional, discovered while discussing the taskbar-badge reinstall problem)
TITLE: Diagnose and fix why the v0.1.4 rebuild STILL showed "Signature not found"
DESC: The v0.1.4 build (34604239872) completed successfully but the build log still showed "Signature not found for the updater JSON. Skipping upload..." on all 3 platforms - the signing/password fix from earlier in this session hadn't actually solved it. Rather than assume the key/secrets were still wrong, tested the exact stored key + password locally via `npx tauri signer sign` against a throwaway test file - it signed successfully on the first try, proving the key and TAURI_SIGNING_PRIVATE_KEY_PASSWORD secret were both correct all along and ruling that out as the cause.
Re-examined the build log more carefully: `.sig` files WERE being generated correctly for every bundle (.msi.sig, .exe.sig, .app.tar.gz.sig, .AppImage.sig etc. all present), but tauri-action's own "Found artifacts:" step only listed the bare installer paths (.msi, .exe) - never their .sig files, despite those files clearly existing on disk in the same log. Searched tauri-action's GitHub repo for existing issues about this exact symptom and found #1262, which documents precisely this: Tauri v2 defaults to a signing mode (`createUpdaterArtifacts: true`, implicit when unset) that signs the installer directly and does NOT produce the legacy `.msi.zip`/`.app.tar.gz`-wrapped copies that tauri-action's artifact-discovery step is hardcoded to look for - so its `.zip.sig` search always comes up empty and it silently skips the upload, even though signing itself succeeded perfectly. Fixed by adding `"createUpdaterArtifacts": "v1Compatible"` to tauri.conf.json's bundle config, which restores generation of the .zip-wrapped artifacts tauri-action expects.
User pushed this fix directly (found via `git log`/`git status` that it was already committed and synced to origin by the time I went to push it myself - no action needed from me there). Re-triggered the release-desktop.yml workflow (run 34605850355); this time the build log showed `.msi.zip`/`.app.tar.gz` being bundled, found, and uploaded successfully with zero "Signature not found" messages anywhere. Confirmed end-to-end by downloading the actual latest.json from the v0.1.4 draft release and inspecting it directly: a complete, well-formed manifest with real per-platform signatures and URLs (windows-x86_64 -> Kinetix_0.1.4_x64_en-US.msi.zip with a genuine signature, matching exactly what the backend endpoint's _TARGET_ALIASES logic expects). The full pipeline (sign -> package updater artifacts -> upload latest.json -> backend dynamically proxies it) now demonstrably works.

TAG: [FEATURE]
PARENT: Fix the desktop auto-update pipeline (was completely non-functional, discovered while discussing the taskbar-badge reinstall problem)
TITLE: Add a manual-update fallback notice for installs that can never verify the new signing key
DESC: User asked directly: since the app is already installed on multiple users, is manually sending everyone the .exe the only option? Explained the actual constraint precisely rather than looking for a workaround that doesn't exist: an already-installed binary has the OLD public key compiled in, and update-signature verification is deliberately one-directional - there is no way to make an old-key binary trust or silently pull a new-key-signed update, by cryptographic design, not a limitation of this specific setup. Asked user how to handle the one-time transition (AskUserQuestion: passive in-app banner vs. active backend-driven notice vs. manual outreach) - user chose the active backend-driven approach.
Investigated what "phones home" already exists to build on rather than invent a new channel: found that the existing Tauri updater plugin ALREADIS calls backend-py's /desktop/update/{target}/{current_version} endpoint (built earlier this session) on every app launch - meaning there's already a natural touchpoint for every client, old or new, no new polling/reporting mechanism needed. The actual gap: AutoUpdateProvider.tsx's checkNativeBinaryUpdate() wrapped update.downloadAndInstall() in a try/catch that silently console.warn'd on ANY failure, with zero user-visible distinction between "no update available" and "update found but this install can never install it" - meaning old-key clients get no experience at all right now, not even a broken one.
Backend (backend-py/app/api/v1/desktop.py): refactored _fetch_latest_manifest() into a shared _fetch_latest_release() that caches both the parsed latest.json AND the release's raw GitHub asset list (previously only the manifest was cached) - needed so a second new endpoint can resolve the actual human-installable file, not just the updater's .zip-wrapped copy. Added GET /desktop/download/{target}, which finds the right plain installer (-setup.exe / .dmg / .AppImage / .deb / .rpm depending on target) from the same live, published-release-driven data source as the update-check endpoint, and 307-redirects to it - so the download link is never a stale/hardcoded URL and updates itself automatically as new versions ship. Updated test_desktop_update.py's cache-reset fixture for the new `assets` cache key and added 3 new tests (Windows/macOS resolution, 404 when no matching asset) - all 10 tests in the file pass.
Frontend (lib/tauri.ts, components/providers/AutoUpdateProvider.tsx): added desktopUpdateTarget(), a UA/platform-based best-effort target-key guesser (deliberately did NOT add @tauri-apps/plugin-os as a new dependency just for this one lookup, per the reuse-existing-code project rule - a UA sniff is good enough since it only needs to point a human at roughly the right installer, it doesn't gate the actual signature verification). Wrapped downloadAndInstall() in its own try/catch: on failure (which, since check() already succeeded, can only really mean the install-time signature verification rejected it - the old-key-mismatch case), shows a persistent, non-dismissible (closeButton: false, duration: Infinity) error toast explaining the install can no longer self-update and linking to the new backend download-redirect endpoint for a one-click manual install. This is the one path that can actually reach users otherwise permanently stranded by the key rotation - after that one manual install, they're back on the new key and auto-update resumes working for them going forward.
...
TAG: [BUG]
TITLE: Emoji picked from the composer/reaction picker didn't match the picker's Apple-style preview
DESC: User reported that the emoji picker (emoji-picker-react, configured with EmojiStyle.APPLE) shows Apple-style emoji images while browsing, but once an emoji is picked and inserted into the chat input, it changes appearance - because insertion just wrote the plain Unicode character, which Windows browsers then render with Windows' own emoji font (Segoe UI Emoji), not Apple's. This is standard OS/browser behavior (raw Unicode carries no font/style info), not a picker bug.
This is an architectural-direction change per this repo's CLAUDE.md rule (ask before shifting architecture) since it means emoji render as images instead of relying on the OS font - confirmed scope with the user via AskUserQuestion at two decision points: (1) leave as-is vs. switch picker to native style vs. render Apple images everywhere -> chose "everywhere"; (2) display-time image swap (stored text stays plain Unicode, only rendering changes) vs. inserting real <img> nodes into the composer that get converted back on send -> chose the display-time-swap approach, but ALSO wanted the composer itself to show the image immediately while typing (not just after sending), which raised the complexity/risk back up to near the second option - confirmed that tradeoff explicitly before proceeding.
Implementation (frontend):
- Added emoji-regex (~10.6.0) as a direct dependency (previously only a transitive dep of eslint/msw tooling with multiple incompatible versions in the tree - unsafe to import as a phantom dependency) for robust emoji-grapheme detection (ZWJ sequences, skin tones, variation selectors).
- New frontend/src/lib/chat/emoji/apple-emoji.tsx: emojiToUnified (Unicode emoji -> emoji-datasource's unified codepoint string, matching the exact CDN URL format emoji-picker-react itself uses internally - https://cdn.jsdelivr.net/npm/emoji-datasource-apple/img/apple/64/{unified}.png - confirmed by reading emoji-picker-react's own source rather than depending on its non-exported internals), appleEmojiImageUrl, findEmojis, createAppleEmojiImage (builds an atomic contenteditable=false <img data-emoji-char="...">), emojifyElement (DOM-walk that swaps emoji text nodes for images in-place, skipping <a>/<button>/already-swapped nodes), and emojifyText (React-node counterpart for plain-React rendering paths, mirrors lib/text/linkify.tsx's linkifyText shape).
- New frontend/src/components/chat/emoji/AppleEmoji.tsx: single-emoji React component wrapping the CDN image, used everywhere a reaction renders (ChatMessageRow, ThreadMessageRow x2, ReactionTooltip, TaskActivityComment).
- Message body display: RichTextPart (HTML-body path, uses dangerouslySetInnerHTML) runs emojifyElement via a ref+useEffect after render. The plain-text path (pure React, no dangerouslySetInnerHTML) instead wires emojifyText into linkify.tsx's linkifyText so link-splitting and emoji-splitting compose in one pass - deliberately did NOT DOM-mutate that path post-render since it's real React-owned children, which would fight React's reconciliation on next re-render (caught this by writing a DOM-mutating version first, mentally tracing the conflict with React on subsequent prop changes, and rewriting before it ever shipped).
- Composer: insertEmoji (hooks/use-rich-composer-field.ts) now calls insertEmojiImageAtCursor(createAppleEmojiImage(emoji)) - a new dom.ts helper mirroring the existing insertChipAtCursor pattern already used for mention chips (atomic contenteditable=false node inserted at the cursor). serializeRichComposerBody (rich-text/serialize.ts) extracts these emoji images back to plain Unicode text via the same marker-and-restore lifecycle extractMentionChips/restoreMentionTokens already used for mention chips (added extractEmojiImages/restoreEmojiTokens, distinct \x01EMOJI{n}\x01 markers) - critically, this conversion happens BEFORE sanitizeMessageHtml runs, so no <img> tag is ever exposed to it. Did NOT widen sanitizeMessageHtml's ALLOWED_TAGS to permit <img> - confirmed via an existing test (markdown-paste.test.ts explicitly asserts <img src=x onerror=...> gets stripped) that <img> exclusion is a deliberate XSS hardening decision, not an oversight, so emoji images must never touch that sanitizer at all. Also wired emojifyElement into restore() (loading an existing message's plain-Unicode emoji back into the composer as images when editing it) and insertQuote() (quoting a message with emoji in it).
- Deliberately did NOT convert LIVE-TYPED or pasted emoji (via OS picker/keyboard, not this app's own picker) into images on every keystroke - that would mean re-walking/mutating the contenteditable DOM on every input event, fighting cursor position after each keystroke, well beyond what was actually reported. Scoped to: picker-inserted, editing-loaded, and quoted-text emoji only.
Two follow-up bugs found and fixed during the user's own manual testing (dev server run locally, credentials not shared with me so user verified visually each round rather than me driving a logged-in browser session):
1. Emoji overlapped the text line above/below - createAppleEmojiImage/AppleEmoji.tsx used a fixed -3px/-4px verticalAlign nudge sized for a 20px image, but the composer's actual line-height is 20px total (text-sm leading-5, i.e. 14px text) - zero margin at that size. Fixed by switching to a relative -0.2em verticalAlign (scales with whatever line-height each context has - composer vs. sent-message text vs. reaction badges - instead of only fitting one specific font size) and reducing the default render size from 20px to 16px.
2. A picked emoji landed at the START of the text instead of at the cursor position - the emoji picker opens in a popover (base-ui), which steals DOM focus/selection the instant it opens, so by the time the user clicks an emoji, window.getSelection() no longer points at their real cursor position. Root-caused by noticing an existing, working analog: saveEditorSelection/restoreEditorSelection (rich-text/dom.ts) already solves this exact class of problem for the formatting toolbar's link-insert popover. Reused it rather than inventing a new mechanism: relaxed saveEditorSelection's isCollapsed guard (added a `requireRange` param, defaulting to false, so it also accepts a plain collapsed caret - the formatting-toolbar caller passes `requireRange: true` to preserve its original stricter behavior) and added saveEmojiInsertPoint to use-rich-composer-field.ts, wired into EmojiPickerPopover's onOpenChange at all 4 composer call sites (MessageComposer, ThreadReplyComposer, InlineMessageEdit, TaskCommentComposer) - saves the cursor the instant the popover opens (before focus moves), insertEmoji restores it right before inserting.
3. (Also fixed proactively while on the overlap bug, same root cause) The composer's placeholder text ("Write a message...") rendered on top of an emoji if the emoji was the ONLY thing in the composer - RichComposerField's showPlaceholder check only looked at draftPlain (el.innerText), and an <img> contributes nothing to innerText (same reason isEmptyComposerHtml already has a separate "<img" string check for its own emptiness test). Fixed by also checking the live DOM for an emoji image (querySelector on the existing EMOJI_IMAGE_DATA_ATTR) in showPlaceholder's condition.
Testing: added jsdom (~30.0.1) as a new devDependency - this repo's vitest.config.ts previously defaulted every test to environment "node" with zero jsdom anywhere in the tree, which meant the DOM-manipulating parts of this feature (emoji image creation/extraction) could only be unit-tested via their no-op "no document" fallback paths, not their real logic - confirmed this gap explicitly with the user (AskUserQuestion) before adding the dependency rather than silently either skipping real coverage or adding jsdom unasked. New test files use the `@vitest-environment jsdom` per-file pragma rather than changing the global default, keeping the rest of the suite on the lighter node environment. Added apple-emoji.test.ts (emojiToUnified codepoint conversion incl. VS16-dropping and ZWJ-sequence handling, appleEmojiImageUrl, findEmojis, emojifyElement DOM behavior) and serialize.test.ts (emoji-image round-trip through serializeRichComposerBody, confirms no <img> ever leaks into the sent body).
Verified: `npx tsc --noEmit` clean throughout every round of fixes, full vitest suite 100/103 passing (same pre-existing 3 unrelated sidebar-display-unread.test.ts failures noted in every prior session - net +13 new passing tests from this feature, 87->100). Committed (06eb704) and pushed to origin/main after explicit user confirmation the visual bugs were fixed.
