...
Verified: `tsc --noEmit` clean, `cargo check` clean (both before adding image-png and after), full vitest suite 87/90 passing (same 3 pre-existing unrelated sidebar-display-unread.test.ts failures from earlier in this repo's history, confirmed unrelated). Not pushed yet per user's standing "don't push until I say" instruction from earlier in this session - committed locally alongside the still-unpushed mobile-responsiveness fix commit from the prior session.
